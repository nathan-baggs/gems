...
RUN sed -i 's/void \*__stack_chk_guard/uintptr_t __stack_chk_guard/' mingw-w64-v${MINGW_VERSION}/mingw-w64-crt/ssp/stack_chk_guard.c || true \
    && sed -i 's/(void\*)//g' mingw-w64-v${MINGW_VERSION}/mingw-w64-crt/ssp/stack_chk_guard.c || true
...
RUN mkdir -p mingw-w64/winpthreads-64 \
    && cd mingw-w64/winpthreads-64 \
    ...
    && make install \
    && cd ../.. \
    && mkdir -p mingw-w64/winpthreads-32 \
    && cd mingw-w64/winpthreads-32 \
    && ../../mingw-w64-v${MINGW_VERSION}/mingw-w64-libraries/winpthreads/configure \
        --prefix=/usr/local/x86_64-w64-mingw32 \
        --host=x86_64-w64-mingw32 \
...
RUN cd gcc \
    && (make -j`nproc` || (cat x86_64-w64-mingw32/32/libatomic/config.log && exit 1)) \
    && make install \
    && cd .. \
